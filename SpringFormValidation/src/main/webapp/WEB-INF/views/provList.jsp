...
<script type="text/javascript" src="<c:url value="/resources/functionsP.js"/>"></script>
...
<body>
	...
	<div id="cos">
		...
		<form:form modelAttribute="ArrayID" name="form" action="delete" method="post">
	  		<form:input type="hidden" path="providers" />
	  	</form:form>
		...
		<div id="personalInfo">
			<h3 class="panel-tittle">
				<strong>List of Providers</strong>
		...
		<table class="table table-bordered" class="table">
			...
			<c:forEach items="${listingP}" var="listingP">
				<tr>
					<td><input type="checkbox" value="${listingP.id}" name="checkbox" onclick="showblocked()"></td>
					<td>${listingP.name}</td>
					<td>${listingP.email}</td>
					<td>${listingP.place}</td>
					<td>${listingP.comentario}</td>
					<td>${listingP.user}</td>
				  	<td>${listingP.password}</td>
					</tr>
			</c:forEach>
		</table>
		</div>
		<div class="panel-footer">
			<div class="rows row">
				<div class="col-md-4">
		  			<button class="trashbut btn btn-danger glyphicon glyphicon-trash" id="deleteProvider" style="display:none"></button>
		  			<button class="modifybut btn btn-info glyphicon glyphicon-user" id="modifyProvider" style="display:none" onClick="check(2)"></button>
		  		</div>
		...
		<ul class="pagination pagination-default">
			...
			<c:choose>
				<c:when test="${npages==1}">
					<li id="color" class="active"><a href="?page=${firstpage}">${firstpage}</a></li>
				</c:when>
		    ...
		    <div class="modal-dialog">
		        <div class="modal-content">
		            ...
		            <div class="modal-body">
		                <p>Delete <span id="modalProvidersNumber"></span> providers?</p>
		            </div>
		            <div class="modal-footer">
		                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
		                <button type="button" class="btn btn-danger" onClick="check(1)">Delete Providers</button>
		            </div>
		        </div>
		    </div>
		</div>
